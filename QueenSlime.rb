...
def playerturn
  ...
  sleep(0.5)
  puts "Queen Slime has #{$bosshp} HP remaining."
  sleep(0.5)
  ...
  if action == "attack" || action == "a" || action == "1"
    ...
    sleep(0.5)
    if attackamount == 1
      puts "You attack Queen Slime with your #{$playerinfo['current_weapon']} #{attackamount} time for #{damage} damage!"
    else
      puts "You attack Queen Slime with your #{$playerinfo['current_weapon']} #{attackamount} times for #{damage} damage!"
    end
    $bosshp -= damage
    if $bosshp <= 0
      puts "You have defeated Queen Slime!"
      $playerinfo['last_boss'] = "queenslime"
...
def bossturn
  system('clear')
  puts "Queen Slime's turn"
  bossattack = rand(1..3)
  if bossattack == 1
    sleep(0.5)
    puts "Queen Slime launches an attack at you..."
    sleep(2)
