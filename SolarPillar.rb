...
def playerturn
  ...
  sleep(0.5)
  puts "Solar Pillar has #{$bosshp} HP remaining."
  sleep(0.5)
  ...
  if action == "attack" || action == "a" || action == "1"
    ...
    sleep(0.5)
    if attackamount == 1
      puts "You attack Solar Pillar with your #{$playerinfo['current_weapon']} #{attackamount} time for #{damage} damage!"
    else
      puts "You attack Solar Pillar with your #{$playerinfo['current_weapon']} #{attackamount} times for #{damage} damage!"
    end
    $bosshp -= damage
    if $bosshp <= 0
      puts "You have defeated Solar Pillar!"
      $playerinfo['max_health'] += 20
      File.open('playerinfo.yml', 'w') { |f| f.write $playerinfo.to_yaml }
      sleep(0.5)
      puts "Now fighting: Nebula Pillar"
      system('ruby NebulaPillar.rb')
    else
      sleep(3)
      bossturn()
    end
...
def bossturn
  system('clear')
  puts "Solar Pillar's turn"
  bossattack = rand(1..3)
  if bossattack == 1
    sleep(0.5)
    puts "Solar Pillar launches an attack at you..."
    sleep(2)
