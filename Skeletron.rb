...
def playerturn
  ...
  sleep(0.5)
  puts "Skeletron has #{$bosshp} HP remaining."
  sleep(0.5)
  ...
  if action == "attack" || action == "a" || action == "1"
    ...
    sleep(0.5)
    if attackamount == 1
      puts "You attack Skeletron with your #{$playerinfo['current_weapon']} #{attackamount} time for #{damage} damage!"
    else
      puts "You attack Skeletron with your #{$playerinfo['current_weapon']} #{attackamount} times for #{damage} damage!"
    end
    $bosshp -= damage
    if $bosshp <= 0
      puts "You have defeated Skeletron!"
      $playerinfo['last_boss'] = "skeletron"
...
def bossturn
  system('clear')
  puts "Skeletron's turn"
  bossattack = rand(1..3)
  if bossattack == 1
    sleep(0.5)
    puts "Skeletron launches an attack at you..."
    sleep(2)
