...
def playerturn
  ...
  sleep(0.5)
  puts "Duke Fishron has #{$bosshp} HP remaining."
  sleep(0.5)
  ...
  if action == "attack" || action == "a" || action == "1"
    ...
    sleep(0.5)
    if attackamount == 1
      puts "You attack Duke Fishron with your #{$playerinfo['current_weapon']} #{attackamount} time for #{damage} damage!"
    else
      puts "You attack Duke Fishron with your #{$playerinfo['current_weapon']} #{attackamount} times for #{damage} damage!"
    end
    $bosshp -= damage
    if $bosshp <= 0
      puts "You have defeated Duke Fishron!"
      $playerinfo['last_boss'] = "dukefishron"
...
def bossturn
  system('clear')
  puts "Duke Fishron's turn"
  bossattack = rand(1..3)
  if bossattack == 1
    sleep(0.5)
    puts "Duke Fishron launches an attack at you..."
    sleep(2)
    puts "...and it grazes you."
    $hp -= 100
    sleep(0.5)
    puts "You lost 100 health..."
    sleep(3)
    system('clear')
    if $hp > 0
      playerturn()
